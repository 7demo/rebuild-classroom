...
var fabric = fabric || { version: "1.6.30" };
...
(function() {
  ...
  fabric.PencilBrush = fabric.util.createClass(fabric.BaseBrush, /** @lends fabric.PencilBrush.prototype */ {
    ...
    initialize: function(canvas) {
      this.canvas = canvas;
      this._points = [ ];
      this.time = undefined;  //SAMPAN 16-4-6
      this.duration = 10;//SAMPAN 16-4-6
    },
    ...
    onMouseDown: function(pointer) {
      ...
      this._render();
      this.time = (new Date().getTime());//SAMPAN 16-4-6
      yscanvas.config.canvasObjectId = user.uid + Date.now();
      sendPomeloData && sendPomeloData({
        type : 'drawFlag',
        data : {
          mouse : 'down'
        }
    ...
    onMouseMove: function(pointer) {
      var __nowtick = (new Date().getTime());
      this._captureDrawingPath(pointer);
      // redraw curve
      // clear top canvas
      ...
      this._render();
      // this.time = (new Date().getTime());//SAMPAN 16-4-6
      var _sendPoints = this.convertPointsToSVGPath(this._points).join(''),
          _sendpath = this.createPath(_sendPoints);
      if (this.time && (__nowtick - this.time)>this.duration) {//SAMPAN 16-4-6
        this._captureDrawingPath(pointer);
        // redraw curve
        // clear top canvas
        this.canvas.clearContext(this.canvas.contextTop);
        ...
        // sendPomeloData({
        //   type : 'draw',
        //   data : JSON.stringify(_sendpath.path)
        // })
        sendPomeloData && sendPomeloData({
          type : 'draw',
          data : {
            x : pointer.x,
            y : pointer.y,
            id : yscanvas.config.canvasObjectId
          }
        })
    ...
    onMouseUp: function() {
      ...
      sendPomeloData && sendPomeloData({
        ...
        //设置当前klass的id
        var _objs = yscanvas.config.canvasArray[yscanvas.config.curentPage][user.page[user.uid]].getObjects();
        _objs[_objs.length - 1].id = yscanvas.config.canvasObjectId;
        // if (user.role != 'teacher') {
        //   _objs[_objs.length - 1].selectable = false; //禁止选择
        // }
        _objs[_objs.length - 1].selectable = yscanvas.config.earserFlag;
        _objs[_objs.length - 1].lockMovementX = false; //禁止选择
        _objs[_objs.length - 1].lockMovementY = false; //禁止选择
        yscanvas.config.canvasObjectId = undefined; //清除id
...
(function() {
  ...
  fabric.Canvas = fabric.util.createClass(fabric.StaticCanvas, /** @lends fabric.Canvas.prototype */ {
    ...
    setCursor: function (value) {
      //sampan  屏蔽自带鼠标样式
      if (!yscanvas.config.isFreeDraw && user.role == 'teacher') {
    ...
    deactivateAll: function () {
      ...
      for ( ; i < len; i++) {
        allObjects[i] && allObjects[i].set && allObjects[i].set('active', false);
      }
      this._discardActiveGroup();
      this._discardActiveObject();
      return this;
...
(function() {
  ...
  fabric.util.object.extend(fabric.Canvas.prototype, /** @lends fabric.Canvas.prototype */ {
    ...
    _onMouseMove: function (e) {
      ...
      var self = this;
      if (!self.mouseSendFlag && user.role == 'teacher') {
        self.mouseSendFlag = setTimeout(function () {
          sendPomeloData && sendPomeloData({
            type : 'mousemove',
            data : {
              x : e.offsetX / yscanvas.config.zoom,
              y : e.offsetY / yscanvas.config.zoom
            }
          })
          self.mouseSendFlag = undefined;
        }, 40);
      }
    },
